...
export class FakeMockDB {
  ...
  private readonly _behandlere = [
    { navn: "Frida Perma Frost", hprNummer: null, telefon: "tel:94431152" },
  ];
  ...
  private readonly _sykmeldinger: Record<
    Sykmeldte,
    [SykmeldingDeduplicated, ...SykmeldingDeduplicated[]]
  > = {
  ...
  public get sykmeldte(): PreviewSykmeldtApi[] {
    return MineSykmeldteApiSchema.parse(
      entries(this._sykmeldte).map(
        ([sykmeldtNavn, sykmeldt]): PreviewSykmeldtApi => {
          const sykmeldtSykmeldinger: SykmeldingApi[] = entries(
            this._sykmeldinger,
          )
            .filter(([sykmeldingNavn]) => sykmeldtNavn === sykmeldingNavn)
            .flatMap(([navn, sykmeldinger]) =>
              sykmeldinger.map((it): [Sykmeldte, SykmeldingDeduplicated] => [
                navn,
                it,
              ]),
            )
            .map(
              ([navn, sykmelding]): SykmeldingApi =>
                toCompleteSykmelding(navn, sykmeldt, sykmelding),
            );
  ...
  public async getSykmelding(
    sykmeldingId: QuerySykmeldingArgs["sykmeldingId"],
  ): Promise<SykmeldingApi> {
    ...
    return SykmeldingSchema.parse(
      toCompleteSykmelding(navn, sykmeldt, sykmelding),
    );
  }

  public async getSoknad(
    soknadId: QuerySoknadArgs["soknadId"],
  ): Promise<SoknadApi> {
    ...
    if (soknad.status !== SoknadsstatusEnum.Sendt) {
      throw new Error(
        "500: Søknad is not sendt or korrigert and should not be fetched using getSoknad",
      );
    }
  ...
  public markHendelseResolved(hendelseId: string): void {
    if (this.hasDialogmote(hendelseId)) {
      ...
      this._dialogmoter[sykmeldt] = this._dialogmoter[sykmeldt].filter(
        (it) => it.hendelseId !== hendelseId,
      );
    } else {
      const [sykmeldt] = this.getOppfolgingsplanById(hendelseId);
      this._oppfolgingsplaner[sykmeldt] = this._oppfolgingsplaner[
        sykmeldt
  ...
  public unlinkSykmeldte(narmestelederId: string): void {
    const sykmeldt = entries(this._sykmeldte).find(
      ([, sykmeldt]) => sykmeldt.narmestelederId === narmestelederId,
    );

    if (!sykmeldt) {
      throw new Error(
        `Unable to find sykmeldt with narmestelederId ${narmestelederId}`,
      );
    }
    ...
    const unreadSoknadTuple = entries(this._soknader).flatMap(
      ([navn, soknader]) =>
        soknader
          ...
          .filter(
            ([, soknad]) =>
              (soknad.status === "NY" || soknad.status === "SENDT") &&
              !soknad.lest,
          ),
    );
  ...
  private getSykmeldingById(
    sykmeldingId: string,
  ): [Sykmeldte, SykmeldingDeduplicated] {
    const sykmeldingTuple: [Sykmeldte, SykmeldingDeduplicated] | undefined =
      entries(this._sykmeldinger)
        .flatMap(([navn, sykmeldinger]) =>
          sykmeldinger.map((it): [Sykmeldte, SykmeldingDeduplicated] => [
            navn,
            it,
    ...
    if (!sykmeldingTuple) {
      throw new Error(
        `404: Unable to find sykmelding with ID ${sykmeldingId} in mock test data`,
      );
    }
  ...
  private getSoknadById(soknadId: string): [Sykmeldte, PreviewSoknadApi] {
    const soknadTuple: [Sykmeldte, PreviewSoknadApi] | undefined = entries(
      this._soknader,
    )
      .flatMap(([navn, soknader]) =>
        soknader.map((it): [Sykmeldte, PreviewSoknadApi] => [navn, it]),
      )
      .find(([, soknad]) => soknad.id === soknadId);

    if (!soknadTuple) {
      throw new Error(
        `404: Unable to find soknad with ID ${soknadId} in mock test data`,
      );
    }
  ...
  private getDialogmoteById(hendelseId: string): [Sykmeldte, DialogmoteApi] {
    const hendelseTuple: [Sykmeldte, DialogmoteApi] | undefined = entries(
      this._dialogmoter,
    )
      .flatMap(([navn, hendelser]) =>
        hendelser.map((it): [Sykmeldte, DialogmoteApi] => [navn, it]),
      )
      .find(([, hendelse]) => hendelse.hendelseId === hendelseId);

    if (!hendelseTuple) {
      throw new Error(
        `404: Unable to find hendelse with ID ${hendelseId} in mock test data`,
      );
    }
  ...
  private getOppfolgingsplanById(
    hendelseId: string,
  ): [Sykmeldte, OppfolgingsplanApi] {
    const hendelseTuple: [Sykmeldte, OppfolgingsplanApi] | undefined = entries(
      this._oppfolgingsplaner,
    )
      .flatMap(([navn, hendelser]) =>
        hendelser.map((it): [Sykmeldte, OppfolgingsplanApi] => [navn, it]),
      )
      .find(([, hendelse]) => hendelse.hendelseId === hendelseId);
  ...
  return {
    ...
    sporsmal: [
            ...
            undersporsmal: [
              {
                ...
                kriterieForVisningAvUndersporsmal:
                  SoknadSporsmalKriterierEnum.Checked,
                svar: [],
                undersporsmal: [
                  {
                    id: "52",
      ...
      },
      {
        ...
        max: null,
        sporsmalstekst:
          "Kvitteringer for reiseutgifter til jobben fra 1. - 24. april 2020.",
        undertekst: null,
        ...
        svar: [
          {
            verdi:
              '{"blobId": "9a186e3c-aeeb-4566-a865-15aa9139d364","belop": 133700,"typeUtgift": "TAXI", "opprettet": ""}',
          },
          {
            verdi:
              '{"blobId": "8653266-aeeb-4566-a865-15aa9139d364","belop": 43200,"typeUtgift": "PARKERING", "opprettet": ""}',
          },
          {
            verdi:
              '{"blobId": "65326165-aeeb-4566-a865-15aa9139d364","belop": 64200,"typeUtgift": "OFFENTLIG_TRANSPORT", "opprettet": ""}',
          },
        ],
        undersporsmal: [],
      },
