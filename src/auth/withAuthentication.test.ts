...
const mockedValidateIdportenToken = oasis.validateToken as unknown as Mock<
...
describe("withAuthentication", () => {
  ...
  describe("withAuthenticatedPage", () => {
    const expectedLoginRedirect =
      "/oauth2/login?redirect=/fake/basepath/test-url";
    ...
    it("should redirect to login when token is missing", async () => {
      ...
      const result: GetServerSidePropsPrefetchResult =
  ...
  describe("withAuthenticatedApi", () => {
    it("should give 401 when token is missing", async () => {
      ...
      expect(fakeResponse.mockJson).toHaveBeenCalledWith({
