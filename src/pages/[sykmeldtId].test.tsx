...
describe("Index page", () => {
  ...
  describe("given more or less than 5 people in org without notifications", () => {
    it("should not display filter when there is less than 5 sykmeldte in an org", async () => {
      ...
      expect(
        screen.queryByRole("combobox", { name: "Vis" }),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByRole("combobox", { name: "Sorter etter" }),
      ).not.toBeInTheDocument();
    });
    ...
    it("should display filters when there are 5 or more in an org", async () => {
      ...
      expect(
        screen.getByRole("textbox", { name: "Søk på navn" }),
      ).toBeInTheDocument();
  ...
  describe("when the filter changes for sykmeldte without notifications", () => {
    describe("specifically names", () => {
      ...
      it("should filter by name", async () => {
        ...
        const nonNotifyingSection = screen.getByRole("region", {
          name: "Sykmeldte uten varsel",
        });
        ...
        createPreviewSykmeldt({
          ...
          sykmeldinger: [
            createSykmelding({
              id: "1",
              perioder: [
                createAktivitetIkkeMuligPeriode({ tom: "2021-09-02" }),
              ],
              lest: true,
            }),
          ],
        ...
        createPreviewSykmeldt({
          ...
          sykmeldinger: [
            createSykmelding({
              id: "2",
              perioder: [
                createAktivitetIkkeMuligPeriode({ tom: "2021-09-03" }),
              ],
              lest: true,
            }),
          ],
        ...
        createPreviewSykmeldt({
          ...
          sykmeldinger: [
            createSykmelding({
              id: "3",
              perioder: [
                createAktivitetIkkeMuligPeriode({ tom: "2021-09-01" }),
              ],
              lest: true,
            }),
          ],
        ...
        createPreviewSykmeldt({
          ...
          sykmeldinger: [
            createSykmelding({
              id: "4",
              perioder: [
                createAktivitetIkkeMuligPeriode({ tom: "2021-09-05" }),
              ],
              lest: true,
            }),
          ],
        ...
        createPreviewSykmeldt({
          ...
          sykmeldinger: [
            createSykmelding({
              id: "5",
              perioder: [
                createAktivitetIkkeMuligPeriode({ tom: "2021-09-04" }),
              ],
              lest: true,
            }),
          ],
    ...
    describe('spesifically "vis" filter, for sykmeldte without notifications', () => {
      ...
      it('should filter by sykmeldt when changing "Vis" to sykmeldt', async () => {
        ...
        await userEvent.selectOptions(
          screen.getByRole("combobox", { name: "Vis" }),
          ["Sykmeldte"],
        );
        await waitFor(() =>
          expect(screen.getByRole("combobox", { name: "Vis" })).toHaveValue(
            "sykmeldte",
          ),
        );
      ...
      describe("filter sykmeldte-per-virksomhet", () => {
        ...
        it("should only display orgnavn as heading once if there is more than one sykmeldt per org", async () => {
          ...
          expect(
            screen.getAllByRole("heading", { name: "Bedrift AS" }),
          ).toHaveLength(1);
          expect(
            screen.getAllByRole("heading", { name: "Firma AS" }),
          ).toHaveLength(1);
          expect(
            screen.queryByRole("heading", { name: "Virksomhet AS" }),
          ).not.toBeInTheDocument();
        });
  ...
  describe("sort by names for sykmeldte with notifications", () => {
    ...
    it("should sort by names when changing to Navn", async () => {
      ...
      await userEvent.selectOptions(
        screen.getByRole("combobox", { name: "Sorter varslinger" }),
        ["Navn"],
      );
    ...
    it("should show nofifiation text in front", async () => {
      ...
      expect(
        await notifyingSection.findAllByText("Ulest sykmelding"),
      ).toHaveLength(5);
    });
  });
  ...
  describe("sort by latest date for sykmeldte with notifications", () => {
    ...
    it("should show nofifiation text in front", async () => {
      ...
      expect(
        await notifyingSection.findByRole("region", { name: /Liten Kake/ }),
      ).toHaveTextContent("Oppfølgingsplan venter");
      expect(
        await notifyingSection.findByRole("region", { name: /Stor Kopp/ }),
      ).toHaveTextContent("Sendt søknad");
      expect(
        await notifyingSection.findByRole("region", { name: /Snerten Kake/ }),
      ).toHaveTextContent("Innkalling til dialogmøte");
      expect(
