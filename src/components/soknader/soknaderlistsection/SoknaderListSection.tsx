...
function SoknaderListSection({
  ...
  return (
    <section
      aria-labelledby={`soknader-list-${cleanId(title)}-header`}
      className="mb-16"
    >
...
function SoknadPanel({
  ...
  const notification =
    soknad.__typename !== "PreviewNySoknad" && isPreviewSoknadNotifying(soknad);
  ...
  const commonProps = {
    ...
    description: soknad.perioder
      .map((it) => getSoknadSykmeldingPeriodDescription(it))
      .join(", "),
    notify: notification,
  };
  ...
  if (soknad.__typename === "PreviewSendtSoknad") {
    return (
      <LinkPanel
        href={`/sykmeldt/${sykmeldtId}/soknad/${soknad.id}`}
        {...commonProps}
      >
