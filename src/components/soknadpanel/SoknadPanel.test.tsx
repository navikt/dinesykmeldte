...
describe("SoknadPanel", () => {
  it("should show information about Soknad", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Oppsummering fra søknaden" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("listitem", { name: "Søknaden er sendt inn av" }),
    ).toHaveTextContent("Test person");
  });
  ...
  describe("SporsmalVarianter", () => {
    it("Should show sporsmal for Behandlingsdager if undersporsmal exists", () => {
      ...
      expect(
        screen.getByRole("listitem", { name: "31. mars - 3. april 2020" }),
      ).toBeInTheDocument();
    ...
    it("Should show sporsmal for JaEllerNei, Checkbox and Tall if svar exists and kriterieForVisningAvUndersporsmal matches svar", () => {
            ...
            sporsmal: [
              {
                ...
                kriterieForVisningAvUndersporsmal:
                  SoknadSporsmalKriterierEnum.Ja,
                svar: [
                  {
      ...
      expect(
        screen.getByRole("listitem", {
          name: "Hva slags type transport brukte du?",
        }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("listitem", {
      ...
      expect(
        screen.getByRole("listitem", { name: "852 kr" }),
      ).toBeInTheDocument();
    });
    ...
    it("Should show sporsmal for CheckboxGruppe if undersporsmal exists", () => {
      ...
      expect(
        screen.getByRole("listitem", {
          name: "Hvilke inntektskilder har du hatt?",
        }),
      ).toBeInTheDocument();

      const sykmeldtFraDetteListItems = screen.getAllByRole("listitem", {
        name: "Er du sykmeldt fra dette?",
      });
      ...
      expect(
        screen.getByText("selvstendig næringsdrivende"),
      ).toBeInTheDocument();
    ...
    it("Should show sporsmal for Dato if svar exists", () => {
            ...
            sporsmal: [
              {
                ...
                kriterieForVisningAvUndersporsmal: null,
                svar: [
                  { __typename: "SoknadSporsmalSvar", verdi: "2020-04-01" },
                ],
                undersporsmal: [],
              },
            ],
      ...
      expect(
        screen.getByRole("listitem", { name: "Når begynte du å jobbe igjen?" }),
      ).toBeInTheDocument();
    ...
    it("Should show sporsmal for Fritekst if svar exists", () => {
      ...
      expect(
        screen.getByRole("listitem", { name: "Er dette et spørsmål?" }),
      ).toBeInTheDocument();
    ...
    it("Should show sporsmal for Land if svar exists", () => {
      ...
      expect(
        screen.getByRole("listitem", { name: "Hvilket land?" }),
      ).toBeInTheDocument();
    ...
    it("Should not show sporsmal for Land if svar is missing", () => {
      ...
      expect(
        screen.queryByRole("listitem", { name: "Hvilket land?" }),
    ...
    it("Should show sporsmal for Undertekst if undertekst exists", () => {
      ...
      expect(
        screen.getByRole("listitem", {
          name: "Før du reiser ber vi deg bekrefte:",
        }),
      ).toBeInTheDocument();
      expect(
        screen.getByText(
    ...
    it("Should show sporsmal for RadioGruppe og Tall if svar exists in undersporsmal", () => {
      ...
      expect(
        screen.getByRole("listitem", { name: "10 prosent" }),
      ).toBeInTheDocument();
    });
    it("Should show sporsmal for Kvittering with 1 svar", () => {
            ...
            sporsmal: [
              {
                ...
                sporsmalstekst:
                  "Kvitteringer for reiseutgifter til jobben fra 1. - 24. mai 2020.",
                undertekst: null,
      ...
      expect(
        screen.getByText("Du lastet opp 1 utgift på 1337 kr"),
      ).toBeInTheDocument();
    });
    ...
    it("Should show sporsmal for Kvittering with 3 svar", () => {
            ...
            sporsmal: [
              {
                ...
                tag: SporsmalTagEnum.Kvitteringer,
                sporsmalstekst:
                  "Kvitteringer for reiseutgifter til jobben fra 16. - 18. mai 2021.",
                undertekst: null,
                svartype: SoknadSporsmalSvartypeEnum.Kvittering,
                min: null,
                max: null,
      ...
      expect(
        screen.getByText("Du lastet opp 3 utgifter på til sammen 1601 kr"),
      ).toBeInTheDocument();
    });
  });
});
