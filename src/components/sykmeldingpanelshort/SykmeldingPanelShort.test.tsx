...
describe("SykmeldingPanelShort", () => {
  it("should show correct info", async () => {
    ...
    const infoSection = within(
      screen.getByRole("region", { name: "Opplysninger fra sykmeldingen" }),
    );
    expect(
      infoSection.getByRole("listitem", { name: "Sykmeldingen gjelder" }),
    ).toHaveTextContent("Test Navn");
  });
  ...
  it("should show utenlandsk sykmelding", async () => {
    ...
    const infoSection = within(
      screen.getByRole("region", {
        name: "Opplysninger fra utenlandsk sykmelding",
      }),
    );
    expect(
      infoSection.getByRole("listitem", { name: "Sykmeldingen gjelder" }),
    ...
    expect(
      infoSection.getByRole("listitem", {
        name: "Landet sykmeldingen ble skrevet",
      }),
    ).toHaveTextContent("Polen");
  });
});
