...
describe("SykmeldteList", () => {
  ...
  it("should show error when unable to fetch", async () => {
    ...
    expect(
      await screen.findByText("Klarte ikke å hente dine sykmeldte"),
    ).toBeInTheDocument();
  });
  ...
  it("should list sykmeldte on successful fetch", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "Kari Normann" }),
    ).toBeInTheDocument();
    expect(
      await screen.findByRole("heading", { name: "Ola Normann" }),
    ).toBeInTheDocument();
  });
  ...
  it("should expand and close the panel when clicked", async () => {
    ...
    await userEvent.click(
      within(screen.getByRole("region", { name: /Ola Normann/ })).getByRole(
        "button",
        { name: "Vis mer" },
      ),
    );
    ...
    expect(
      screen.getByRole("link", { name: /Sykmeldinger/ }),
    ).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /Søknader/ })).toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: /Dialogmøter/ }),
    ).toBeInTheDocument();

    await userEvent.click(
      within(screen.getByRole("region", { name: /Ola Normann/ })).getAllByRole(
        "button",
        { name: "Vis mer" },
      )[0],
    );
    ...
    expect(
      screen.queryByRole("link", { name: /Sykmeldinger/ }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("link", { name: /Søknader/ }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("link", { name: /Dialogmøter/ }),
    ).not.toBeInTheDocument();
  });
  ...
  it("should filter by the active virksomhet", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Mr. Show" }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("heading", { name: "Ms. Hide" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("should group sykmeldte by notifying status", async () => {
    ...
    const notifyingSection = within(
      screen.getByRole("region", { name: "Varslinger" }),
    );
    expect(notifyingSection.getAllByRole("button")).toHaveLength(2);
    expect(
      notifyingSection.getByRole("button", { name: /Merk varsler som lest/ }),
    ).toBeInTheDocument();
    await userEvent.click(
      within(screen.getByRole("region", { name: /Mr. Notifying/ })).getByRole(
        "button",
        { name: "Vis mer" },
      ),
    );

    const nonNotifyingSection = within(
      screen.getByRole("region", { name: "Sykmeldte uten varsel" }),
    );
    expect(nonNotifyingSection.getAllByRole("button")).toHaveLength(1);
    expect(
      nonNotifyingSection.getByRole("region", { name: /Ms. Read/ }),
    ).toBeInTheDocument();
  });
  ...
  it("should automatically expand focused sykmeldt based on URL", async () => {
    ...
    expect(
      within(
        await nonNotifyingSection.findByRole("region", { name: /Not focused/ }),
      ).getByRole("button", {
        name: "Vis mer",
      }),
    ...
    expect(
      within(
        await nonNotifyingSection.findByRole("region", { name: /F. Ocused/ }),
      ).getAllByRole("button", {
        name: "Vis mer",
      })[0],
  ...
  it("should automatically set URL to root when closing focused sykmeldt", async () => {
    ...
    await userEvent.click(
      within(
        nonNotifyingSection.getByRole("region", { name: /F. Ocused/ }),
      ).getAllByRole("button", {
        name: "Vis mer",
      })[0],
    );
  ...
  it("should notify user when sykmelding is unread AND missing sendtTilArbeidsgiverDato", () => {
    ...
    expect(
      notifyingSection.getByRole("region", { name: /Buggy Mann/ }),
    ).toBeInTheDocument();
  });
...
function refetchCompleteMock(
  mineSykmeldte: PreviewSykmeldtFragment[],
  refetchComplete: Mock,
): MockedResponse {
  return createMock({
    request: { query: MineSykmeldteDocument },
    result: () => {
