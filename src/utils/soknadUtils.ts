...
export function isPreviewSoknadNotifying(
...
export function getSoknadSykmeldingPeriodDescription(
  period: SoknadperiodeFragment,
): string {
  ...
  switch (period.sykmeldingstype) {
    ...
    case PeriodeEnum.Gradert:
      if (period.sykmeldingsgrad == null)
        throw new Error("Soknadsperiode of type Gradert without grad");
...
export function getSoknadTallLabel(
...
export function soknadByDateDesc(
  a: PreviewSoknadFragment,
  b: PreviewSoknadFragment,
): number {
  return compareDesc(toDate(a.tom), toDate(b.tom));
}

export function previewNySoknaderRead(
  soknader: PreviewSoknadFragment[],
): PreviewSoknadFragment[] {
  const previewNySoknad = soknader.filter(
    (it) =>
      it.__typename === "PreviewNySoknad" &&
      it.ikkeSendtSoknadVarsel &&
      it.lest,
  );
...
export function previewNySoknaderUnread(
  soknader: PreviewSoknadFragment[],
): PreviewSoknadFragment[] {
  const previewNySoknad = soknader.filter(
    (it) =>
...
export function previewSoknadUnreadCount(soknader: PreviewSoknadFragment[]): {
  ...
  const sendtSoknadCount = soknader.filter(
...
export function getSoknadNotifyDescription(
  soknader: PreviewSoknadFragment[],
): string[] | null {
  const { sendtSoknadCount, nySoknadCount } =
    previewSoknadUnreadCount(soknader);
  if (soknader.length === 0) return null;

  const sendtSoknadTekst =
    sendtSoknadCount === 1
      ? "1 sendt søknad"
      : `${sendtSoknadCount} sendte søknader`;
  const nySoknadTekst =
    nySoknadCount === 1
      ? "Vi mangler 1 søknad"
      : `Vi mangler ${nySoknadCount} søknader`;
